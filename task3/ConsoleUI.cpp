...
ConsoleUI::ConsoleUI(std::shared_ptr<CentralBank> cb) : centralBank(cb) {
}
...
void ConsoleUI::performTransaction() {
    if (!currentBank || !currentClient) {
        std::cout << "Please select a bank and create a client first!\n";
        return;
    }
...
void ConsoleUI::viewTransactionHistory() {
    if (!currentBank) {
        std::cout << "Please select a bank first!\n";
        return;
    }
...
void ConsoleUI::rollbackTransaction() {
    if (!currentBank) {
        std::cout << "Please select a bank first!\n";
        return;
    }
